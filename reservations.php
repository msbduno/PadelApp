...
<body>
    ...
    <div class="fond2">
        ...
        $tournois = array_map(function ($i) use ($startDate) {
            return (clone $startDate)->add(new DateInterval("P{$i}D"));
        }, range(0, 2));
        ...
        print("<tr class=terrains><th>Heure</th>"); foreach ($tournois as $day) foreach ($terrains as $terrain)
                print("<th colspan=1>" . $terrain . "</th>");
        print("</tr>"); foreach ($times as $time => $value) {
            ...
            foreach ($tournois as $day) {
                ...
                foreach ($tournois2024 as $date => $event) {
                    ...
                    if ($day->format('md') == $datetime->format('md')) {
                        if ($time == '8h00')
                            print("<td colspan=4 rowspan=34 class=tournoi>" . $event . "</td>");
                        goto out;
                    }
                }
                foreach ($terrains as $terrain) {
                    ...
                    if (!$filled) {
                        ...
                        //$onclickEvent = "handleCellClick('" . $classname . "');";
                        print("<td " . $isModalActive . " class=" . $classname . " id=" . $id . " auto;\" onclick=\"handleCellClick('" . $classname . "');\"></td>");
                    }
...
<script>
    function handleCellClick(className) {
        // Ajoutez ici le code que vous souhaitez exécuter lors du clic sur la cellule.
        //console.log('Clic sur la cellule avec la classe : ' + className);


        if (onclick = true) {
            alert("Réservation éffectuée !");
            <?php
            $idUtilisateur = $_SESSION['compte'];

            $sqlRequete = "INSERT INTO reservation
        ...
        estPaye = '0'";
            ...
            $resultRequete = $conn->query($sqlRequete);
